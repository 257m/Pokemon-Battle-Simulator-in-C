   ...
   if (!EnemySwitch) {
        ...
        printf("Your %s is at %d/%d hp\n",PlayerParty.P1->Poke->Name,PlayerParty.P1->CurrentHp,PlayerParty.P1->Hp);
          ...
          if (EnemyTurn->Move->BoostRate[6] != 0) {
             if (EnemyTurn->Move->BoostRate[6] != 0) printf("%s Evasion rose by %d stages!\n",EnemyParty.P1->Poke->Name,EnemyTurn->Move->BoostRate[6]);
            else printf("%s Evasion Chance fell by %d stages!\n",EnemyParty.P1->Poke->Name,EnemyTurn->Move->BoostRate[6]);
          }  
          if (EnemyTurn->Move->BoostRate[7] != 0) {
             if (EnemyTurn->Move->BoostRate[7] != 0) printf("%s Crit Chance rose by %d stages!\n",EnemyParty.P1->Poke->Name,EnemyTurn->Move->BoostRate[7]);
            else printf("%s Crit Chance fell by %d stages!\n",EnemyParty.P1->Poke->Name,EnemyTurn->Move->BoostRate[6]);
          }
