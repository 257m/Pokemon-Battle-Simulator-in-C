void display_move(bool eop) {
        if (Parties[eop].CanMove) {
          printf("%s%s used %s!\n",EOPTEXT[eop],str_decompress_and_format_free(POKEMONDEX[Parties[eop].Member[0]->Poke].Name),str_decompress_and_format_free(MoveList[Parties[eop].Turn->Move].Name));
