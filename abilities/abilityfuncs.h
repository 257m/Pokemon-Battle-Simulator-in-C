...
void TypeChange(char et,bool eop) {
  if (et == 2) {
    ...
    Parties[eop].TemporaryMult *= TypeChart[((AbilityList[Parties[eop].Member[0]->Ability].GNRL_PURPOSE[0] << 5) >> 3) + (AbilityList[Parties[eop].Member[0]->Ability].GNRL_PURPOSE[1] >> 6)][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type1] * TypeChart[((AbilityList[Parties[eop].Member[0]->Ability].GNRL_PURPOSE[0] << 5) >> 3) + (AbilityList[Parties[eop].Member[0]->Ability].GNRL_PURPOSE[1] >> 6)][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type2];
    } else {
