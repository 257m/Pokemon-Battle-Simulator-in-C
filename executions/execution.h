...
void ExecuteMove(bool eop) {
  if (!eop) {
    if (!Dead[eop]) {
        if (!PlayerSwitch) {
        Hit[0] = (rand() < RAND_MAX*((double)(MoveList[Parties[0].Turn->Move].Accuracy*statboostmult(Parties[0].Member[0]->StatBoosts[5] / statboostmult(Parties[1].Member[0]->StatBoosts[6])))/100));
        if (MoveList[Parties[0].Turn->Move].Accuracy > 100) Hit[0] = 1;
  ...
  if (!(CHK_BIT(Parties[1].EFFECT_FLAGS[0],EFFECT_PROTECT) && CHK_BIT(MoveList[Parties[0].Turn->Move].FLAGS,nFLAG_PROTECT_AFFECTED))) {
    ...
    if (Dead[1] == 0) {
        if (EnemySwitch == 0) {
        Hit[1] = (rand() < RAND_MAX*((double)(MoveList[Parties[1].Turn->Move].Accuracy*statboostmult(Parties[1].Member[0]->StatBoosts[5] / statboostmult(Parties[0].Member[0]->StatBoosts[6])))/100));
        if (MoveList[Parties[1].Turn->Move].Accuracy > 100) Hit[1] = 1;
