void endturn(bool eop) {
    ...
    if (Parties[eop].Member[0]->Non_Volatile_Status == STATUS_BURN) {
        UDBOG2(&Parties[eop].Member[0]->CurrentHp,Parties[eop].Member[0]->Hp/16,eop,STATUS_BURN);
      } else if (Parties[eop].Member[0]->Non_Volatile_Status == STATUS_POISON) {
        UDBOG2(&Parties[eop].Member[0]->CurrentHp,Parties[eop].Member[0]->Hp/8,eop,STATUS_POISON);
